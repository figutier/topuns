...
#Modification from Theo Lacombe's implementation of Lacombe et al. (2021) at https://github.com/tlacombe/topologicalUncertainty/blob/master/utils/utils_graphs.py 
...
def _get_vertices_and_edges_values_from_input(model, x_train, layers_id=None, only_fc=True):
    ...
    if layers_id is None:
        ...
        layers_id = np.arange(len(layers))

    if only_fc:
        for id, layer in enumerate(module_list):
...
def build_graphs_from_deep_model(model, x_train, layers_id=None):
    '''
    Given a torch sequential model, a set of observations and a subset of layers to consider,
    compute a list of list of graphs (encoded as SimplexTree) for the different layers and observations.

    :param model: A torch (sequential) model.
